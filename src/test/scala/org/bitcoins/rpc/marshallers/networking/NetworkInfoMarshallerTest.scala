...
import org.bitcoins.rpc.bitcoincore.networking.NetworkInfo
...
class NetworkInfoMarshallerTest extends FlatSpec with MustMatchers{
  ...
  val network : NetworkInfo = NetworkRPCMarshaller.NetworkInfoFormatter.read(json)
  ...
  it must "write network info" in {
    val writtenNetworkInfo = NetworkRPCMarshaller.NetworkInfoFormatter.write(network)
