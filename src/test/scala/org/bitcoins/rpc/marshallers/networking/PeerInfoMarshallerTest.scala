...
import org.bitcoins.rpc.bitcoincore.networking.PeerInfo
...
class PeerInfoMarshallerTest extends FlatSpec with MustMatchers {
  ...
  val peer : PeerInfo = PeerInfoRPCMarshaller.PeerInfoFormatter.read(json)
  ...
  it must "write peer info" in {
    val writtenPeerInfo = PeerInfoRPCMarshaller.PeerInfoFormatter.write(peer)
